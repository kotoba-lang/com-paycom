(ns paycom.main
  "Kotodama WASM entrypoint for the Paycom clean-room actor (L4) — Clojure port.
  ...
  facts (`paycom.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "paycom")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"paycom.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "paycom-compat" :tier tier :entities entities} 200])
...
(defn start! [] :paycom-compat/ready)
